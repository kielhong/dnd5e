...
import com.widehouse.dnd.item.DamageType.*
...
class WeaponTest : FunSpec({
    test("Weapon property") {
        val weapon = Weapon("dagger", Simple, Melee, listOf(Dice.D4), Piercing, cost = Coin(2, GP), weight = 1)
        assertSoftly(weapon) {
    ...
    test("Weapon damage roll") {
        ...
        sword = Weapon("sword", damage = listOf(Dice.D6, Dice.D6), damageType = Bludgeoning)
