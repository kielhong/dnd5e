...
class AttackTest : FunSpec({
    test("Character attack target then target get damage") {
        ...
        val target = Character(ability = mapOf(), 2, hitPoints = 20)
    ...
    test("Attack Roll then hit") {
        val dice = mockk<Dice>()
        val char = Character(ability = mapOf("str" to Strength(18)), 3, hitPoints = 20, dice = dice)
    ...
    test("Damage Roll") {
        ...
        val char = Character(ability = mapOf("str" to Strength(15)), 2, hitPoints = 20, weapon = sword)
